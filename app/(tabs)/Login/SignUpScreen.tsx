...
export default function SignUpScreen({ navigateTo }) {
  ...
  const handleSendOtp = () => {
    ...
    axios.post('http://localhost:3000/reqOTP', { email })
      .then(() => {
        setIsOtpSent(true);
  ...
  const handleVerifyOtpAndSignUp = () => {
    ...
    axios.post('http://localhost:3000/verifyOTP', { email, otp, name, password: pass })
      .then((res) => {
        alert(res.data.message);
        if (res.data.message === 'OTP Verified and User Registered Successfully') {
          // Reset fields and navigate to home or login page
          setName('');
          setEmail('');
          setPass('');
          setOtp('');
          navigateTo('verify'); // Use navigateTo prop for navigation
        }
      })
      .catch((err) => {
        console.error(err);
      ...
      {!isOtpSent ? (
        ...
        <View>
          <TextInput
            ...
            style={styles.input}
          />
          <Button title="Verify OTP and Sign Up" onPress={handleVerifyOtpAndSignUp} />
        </View>
      )}
...
export default function SignUpScreen({ navigateTo }) {
  ...
  const handleSubmit = () => {
    ...
    axios.post('http://localhost:3000/', { name, email, pass, selectedRole })
      .then((res) => {
        ...
        setEmail('');
        setPass('');
        setSelectedRole('Worker'); // Reset selected role
        navigateTo('verify'); // Use navigateTo prop for navigation
      })
      .catch((err) => {
        console.log(err);
  ...
  useEffect(() => {
    ...
    setEmail('');
    setPass('');
    setSelectedRole('Worker'); // Reset role on mount
  }, []);
  ...
  return (
    <View style={[styles.container, isLargeScreen && styles.largeScreenContainer]}>
     <Image
      ...
      {ress ? <Text style={styles.responseText}>{ress}</Text> : null}
    </View>
  );
}
...
//         <View style={{marginTop: 5,}}></View>
//         <TextInput placeholder="Valid email" style={styles.input} />
//         <View style={{marginTop: 5,}}></View>
...
//         </Text>
//       </View>
//       <TouchableOpacity style={styles.button} onPress={()=>navigateTo('verify')}>
//         <Text style={styles.buttonText} >Next</Text>
//       </TouchableOpacity>
//       <Text style={styles.footerText}>
//         Already a member? <Text style={styles.linkText} onPress={()=>navigateTo('login')}>Log In</Text>
//       </Text>
//     </View>
//   );
// }
...
//   },
//   largeScreenContainer: {
//     paddingHorizontal: 50, // Increased padding for larger screens
//   },
//   image: {
//     width: 100,
//     height: 100,
