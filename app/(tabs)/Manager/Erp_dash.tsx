import React from 'react';
import { View, Text, FlatList,Image, TouchableOpacity, StyleSheet, ScrollView } from 'react-native';
import styles  from './Erp_styles';
const equipmentData = [
  { name: 'Excavator A', status: 'Active', location: 'Pit 3' },
  { name: 'Dump Truck', status: 'Under repair', location: 'Garage' },
  { name: 'Conveyor A', status: 'Active', location: 'Main conveyor' },
  { name: 'Crane A', status: 'Active', location: 'Site 2' },
  { name: 'Driller', status: 'Under repair', location: 'Garage' },
];

const materialData = [
    { material: 'Explosives', quantity: '500 kg', status: 'Secure' },
    { material: 'Fuel', quantity: '2000 L', status: 'Low stock' },
    { material: 'Spare parts', quantity: '300 pcs', status: 'Sufficient' },
    { material: 'Wires', quantity: '2000 m', status: 'Sufficient' },
    { material: 'Driller', quantity: '10 set', status: 'Secure' },
  ];
  
  const safetyData = [
    { date: '12-08-2024', inspector: 'Majid', area: 'Site - 2', findings: 'No Issues' },
    { date: '12-08-2024', inspector: 'Sam', area: 'Garage', findings: 'Minor repairs' },
    { date: '12-08-2024', inspector: 'Rahul', area: 'Main pit', findings: 'No Issues' },
  ];

const Erp_Dashboard = ({navigateTo}) => {
  return (
    <View contentContainerStyle={styles.container}>
         <TouchableOpacity  onPress={()=>navigateTo('Manager')}>
          <Text style={styles.backText}>{"<"}</Text>
        </TouchableOpacity>
      <ScrollView contentContainerStyle={styles.container}>
      {/* Header */}
      <View style={styles.header}>
      <Text style={styles.headertext}>ERP Dashboard</Text>
      </View>
      {/* Equipment Management and Inventory Control */}
      <View style={styles.tableHeader}>
        <Text style={[styles.cell, styles.headerCell]}>Equipment</Text>
        <Text style={[styles.cell, styles.headerCell]}>Status</Text>
        <Text style={[styles.cell, styles.headerCell]}>Current Location</Text>
        <Text style={[styles.cell, styles.headerCell]}>Actions</Text>
      </View>

      <FlatList
        data={equipmentData}
        keyExtractor={(item) => item.name}
        renderItem={({ item }) => (
          <View style={styles.row}>
            <Text style={styles.cell}>{item.name}</Text>
            <Text style={styles.cell}>{item.status}</Text>
            <Text style={styles.cell}>{item.location}</Text>
            <View style={styles.actions}>
              <TouchableOpacity style={styles.actionButton}>
                <Text style={styles.actionText}>View</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.actionButton}>
                <Text style={styles.actionText}>Edit</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
      />

      {/* Buttons */}
      <View style={styles.buttonRow}>
        <TouchableOpacity style={styles.mainButton}>
          <Text style={styles.buttonText}>Add Equipment</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.mainButton}>
          <Text style={styles.buttonText}>View Maintenance</Text>
        </TouchableOpacity>
      </View>

      {/* Production and Financial Report */}
      <View style={styles.section}>
      <View style={styles.header}>
      <Text style={styles.headertext}>Production Financial Report</Text>
      </View>
        <View style={styles.tableHeader}>
          <Text style={[styles.cell, styles.headerCell]}>Daily output (Tons)</Text>
          <Text style={[styles.cell, styles.headerCell]}>Investment</Text>
          <Text style={[styles.cell, styles.headerCell]}>Revenue</Text>
          <Text style={[styles.cell, styles.headerCell]}>Profit/Loss</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.cell}>5000</Text>
          <Text style={styles.cell}>70,000 $</Text>
          <Text style={styles.cell}>1,00,000 $</Text>
          <Text style={styles.cell}>30,000 $</Text>
        </View>

        {/* Report Buttons */}
        <View style={styles.buttonRow}>
          <TouchableOpacity style={styles.mainButton}>
            <Text style={styles.buttonText}>View Full Report</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.mainButton}>
            <Text style={styles.buttonText}>Download Report</Text>
          </TouchableOpacity>
        </View>
      </View>


      <View style={styles.section}>
        <View style={styles.tableHeader}>
          <Text style={[styles.cell, styles.headerCell]}>Material</Text>
          <Text style={[styles.cell, styles.headerCell]}>Quantity</Text>
          <Text style={[styles.cell, styles.headerCell]}>Status</Text>
          <Text style={[styles.cell, styles.headerCell]}>Actions</Text>
        </View>

        <FlatList
          data={materialData}
          keyExtractor={(item) => item.material}
          renderItem={({ item }) => (
            <View style={styles.row}>
              <Text style={styles.cell}>{item.material}</Text>
              <Text style={styles.cell}>{item.quantity}</Text>
              <Text style={styles.cell}>{item.status}</Text>
              <View style={styles.actions}>
                <TouchableOpacity style={styles.actionButton}>
                  <Text style={styles.actionText}>View</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.actionButton}>
                  <Text style={styles.actionText}>Edit</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
        />

        {/* Buttons */}
        <View style={styles.buttonRow}>
          <TouchableOpacity style={styles.mainButton}>
            <Text style={styles.buttonText}>Add Materials</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.mainButton}>
            <Text style={styles.buttonText}>View Storage</Text>
          </TouchableOpacity>
        </View>
      </View>

      {/* Safety and Compliance */}
      <View style={styles.section}>
      <View style={styles.header}>
      <Text style={styles.headertext}>Safety and Compliance</Text>
      </View>
        <View style={styles.tableHeader}>
          <Text style={[styles.cell, styles.headerCell]}>Inspection Date</Text>
          <Text style={[styles.cell, styles.headerCell]}>Inspector Name</Text>
          <Text style={[styles.cell, styles.headerCell]}>Area</Text>
          <Text style={[styles.cell, styles.headerCell]}>Findings</Text>
        </View>
        </View>

        <FlatList
          data={safetyData}
          keyExtractor={(item) => item.date + item.inspector}
          renderItem={({ item }) => (
            <View style={styles.row}>
              <Text style={styles.cell}>{item.date}</Text>
              <Text style={styles.cell}>{item.inspector}</Text>
              <Text style={styles.cell}>{item.area}</Text>
              <Text style={styles.cell}>{item.findings}</Text>
            </View>
          )}
        />

        {/* Full Records Button */}
        <View style={styles.buttonRow}>
          <TouchableOpacity style={styles.fullButton}>
            <Text style={styles.buttonText}>View Full Records</Text>
          </TouchableOpacity>
        </View>

        </ScrollView>


      {/* Footer */}
      <View style={styles.footer}>
      <TouchableOpacity>
          <Image source={{ uri: 'data:image/png;base64,' }} style={styles.footerIcon} />
          <Text style={styles.footertext}>Home</Text>
        </TouchableOpacity>
        <TouchableOpacity>
          <Image source={{ uri: 'data:image/png;base64,' }} style={styles.footerIcon} />
          <Text style={styles.footertext}>Report</Text>
        </TouchableOpacity>
        <TouchableOpacity>
          <Image source={{ uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTLs98eObr7yGomTrCyGxd0OPX3e3GR-SLvJQ&s' }} style={styles.footerIcon} />
          <Text style={styles.footertext}>Alert</Text>
        </TouchableOpacity>
        <TouchableOpacity>
          <Image source={{ uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ0hO_uKSh2znmEXW70dnhO8AU7l8--5b_fWw&s' }} style={styles.footerIcon} />
          <Text style={styles.footertext}>ERP</Text>
        </TouchableOpacity>
        <TouchableOpacity>
          <Image source={{ uri: 'data:image/jpeg;base64,' }} style={styles.footerIcon} />
          <Text style={styles.footertext}>Profile</Text>
        </TouchableOpacity>
      </View>
      </View>
   );
};

export default Erp_Dashboard;